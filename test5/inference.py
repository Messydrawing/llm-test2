...
from .graphrag_utils import search as graphrag_search
...
def run(
    ...
    *,
    rag_root: str = "graphrag_project",
    rag_config: str | None = None,
    rag_data: str | None = None,
) -> dict[str, str]:
    """拼 Prompt → 调 teacher & student → 返回 dict 结果"""
    rag_context = graphrag_search(
        question,
        root_dir=rag_root,
        config=rag_config,
        data_dir=rag_data,
    )
    prompt = question + "\n" + summarize_stock(question) + rag_context + "\n"
...
def main() -> None:
    ...
    ap.add_argument(
        "--rag-config",
        help="GraphRAG settings.yaml path",
    )
    ap.add_argument(
        "--rag-data",
        help="Path to GraphRAG index parquet directory",
    )
    args = ap.parse_args()
