...
def _make_prompt(window) -> dict[str, Any]:
    """Return initial record structure for a sampled window."""
...
def format_prompt(sample: dict[str, Any]) -> str:
    """Convert a dataset sample into the textual prompt for the teacher."""
    lines = []
    for i, day in enumerate(sample["kline_summary"], 1):
        line = (
            f"Day{i}: O={day['open']}, H={day['high']}, "
            f"L={day['low']}, C={day['close']}"
        )
        lines.append(line)
    summary = "; ".join(lines)
    return (
        f"过去30日K线数据如下：{summary}。"
        "基于以上数据，预测未来3日走势并给出分析和操作建议。"
        "请以JSON格式回答。"
    )
    ...
    *,
    ...
    for code in codes:
        ...
        for i in range(n - window - 3 + 1):
            ...
            future = df.iloc[i + window : i + window + 3]
            future_close = future["close"].mean()
            base_close = win["close"].iloc[-1]
            fut_change = (future_close / base_close - 1) * 100
            prompt["target"] = "涨" if fut_change >= 0 else "跌"
